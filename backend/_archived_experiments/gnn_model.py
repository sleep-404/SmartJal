...
class GroundwaterGNN(nn.Module):
    ...
    def __init__(self,
                 ...
                 heads: int = 2):
        """
        Simplified GNN for small datasets (~138 training nodes).

        Key design choices for avoiding overfitting:
        - Small hidden dimension (16 instead of 64)
        ...
        # Simple graph convolution layers (GCN is more stable than GAT for small data)
        self.convs = nn.ModuleList()
        self.norms = nn.ModuleList()

        for i in range(num_layers):
            # Use simpler GCNConv for small datasets - more stable than GAT
            self.convs.append(GCNConv(hidden_channels, hidden_channels))
            self.norms.append(nn.LayerNorm(hidden_channels))

        # Simple output layer (no MLP to reduce parameters)
        self.output = nn.Linear(hidden_channels, out_channels)

        # Uncertainty: simple linear layer
...
class GNNPredictor:
    ...
    def __init__(self,
                 ...
                 epochs: int = 100):
        """
        Conservative defaults for small dataset (~138 training nodes).
        """
    ...
    def fit(self,
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode='min', factor=0.5, patience=10  # Faster LR reduction
        )
